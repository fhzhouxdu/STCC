...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, control_key, only_mid_control, global_average_pooling=False, stage="train", *args, **kwargs):
        ...
        # weight predict
        # self.weight_predictor = CrossAttentionWeightPredictor(dim=1024, nhead=8)
    ...
    def apply_model(self, x_noisy, t, cond, mode='c_ada', impath=None, *args, **kwargs):
        ...
        if self.stage == "train":
            ...
            #     cond_fus = torch.tile(cond_img, (1, 77, 1))
            # elif random_number == 2:
            #     cond_fus = (cond_txt + cond_img)/2
        else:
            cond_txt = cond_emp
            cond_txt = self.shared_encoder(cond_txt)
            cond_txt = F.normalize(cond_txt, dim=1)
        ...
        # print(self.cond_stage_model.model.encode_image(torch.cat(cond['c_concat'], 1)).shape)
        ...
        if cond['c_concat'] is None:
            ...
            eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_emp, control=control, Y=y, only_mid_control=self.only_mid_control)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.stage == "train":
            ...
            loss_dict.update({f'{prefix}/loss_unceratinty': self.uncertainty_loss})
    ...
    def configure_optimizers(self):
        ...
        for module in self.model.diffusion_model.output_blocks.named_modules():
            if isinstance(module, PixelAwareTransformer):
                pix_params += module.parameters()
...
class CrossAttentionWeightPredictor(nn.Module):
    def __init__(self, dim, nhead):
        super(CrossAttentionWeightPredictor, self).__init__()
        self.cross_attention = nn.MultiheadAttention(embed_dim=dim, num_heads=nhead,batch_first=True)
        self.output_layer = nn.Linear(dim, 2)  # 输出两个权重

    def forward(self, emb_txt, emb_img):

        # 进行交叉注意力计算，这里让x1作为查询，x2作为键和值（也可反过来尝试）
        attn_output, _ = self.cross_attention(emb_txt, emb_img, emb_img)
        
        # 归一化
        avg_pool = nn.AdaptiveAvgPool2d((1, 1024))
        attn_output = avg_pool(attn_output)
        attn_output = attn_output.squeeze(1)
        
        # 经过输出层得到两个权重，并通过softmax保证权重和为1
        weights = F.softmax(self.output_layer(attn_output), dim=1)
        return weights[:, 0].unsqueeze(1).unsqueeze(2), weights[:, 1].unsqueeze(1).unsqueeze(2)